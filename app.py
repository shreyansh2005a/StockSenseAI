"""
=============================================================
  StockSense AI — Streamlit UI
  Drives model.py via run_pipeline(config)
...
.mc-blue::after{background:#58a6ff}.mc-green::after{background:#3fb950}.mc-red::after{background:#f85149}.mc-yellow::after{background:#d29922}
...
.mc-blue .mc-value{color:#58a6ff}.mc-green .mc-value{color:#3fb950}.mc-red .mc-value{color:#f85149}.mc-yellow .mc-value{color:#d29922}
...
def make_price_chart(df, result, ticker, future_days):
    ...
    style_ax(ax, f"{ticker} — Actual vs Predicted Close Price")
    ...
    ax.plot(td, result["y_pred"], color=C["red"],   lw=1.5, linestyle="--", label="Predicted")
    fd = pd.bdate_range(start=dates[-1]+pd.Timedelta(days=1), periods=future_days)
    ax.plot(fd, result["future"], color=C["yellow"], lw=2,
            linestyle=":", marker="o", markersize=2.5, label=f"{future_days}-Day Forecast")
...
def make_indicator_chart(df):
    ...
    ax.plot(df.index, df["RSI"], color=C["blue"], lw=1.2)
    ...
    ax.fill_between(df.index, 70, df["RSI"], where=(df["RSI"]>70), color=C["red"],   alpha=0.1)
    ax.fill_between(df.index, 30, df["RSI"], where=(df["RSI"]<30), color=C["green"], alpha=0.1)
    ax.set_ylim(0, 100)
    ...
    ax.bar(df.index, hist, color=[C["green"] if v>=0 else C["red"] for v in hist], width=1, alpha=0.6)
    ...
    ax.xaxis.set_major_formatter(mdates.DateFormatter("%Y"))
    fig.patch.set_facecolor(C["bg"]); return fig
...
with st.sidebar:
    ...
    seq_len     = st.slider("Look-back Window (days)",  30, 120,  90)
    ...
    with st.expander("Advanced Hyperparameters"):
        ...
        lstm_l1    = st.number_input("LSTM Layer 1",  64, 512, 256, step=64)
        lstm_l2    = st.number_input("LSTM Layer 2",  32, 256, 128, step=32)
    ...
    st.markdown("""<div style="margin-top:1.5rem;font-size:.67rem;color:#3a4a57;
        ...
        MODEL : Bidirectional LSTM<br>LOSS  : Huber<br>
        OPT   : Adam + LR Decay<br>FEATS : 17 (OHLCV+TA+Mom)
    </div>""", unsafe_allow_html=True)


# ─────────────────────────────────────────────
# HERO BANNER — live updates with sidebar values
# ─────────────────────────────────────────────
st.markdown(f"""
<div class="hero-banner">
  <div class="hero-title">STOCKSENSE AI</div>
  <div class="hero-sub">Real-Time Series Analysis &nbsp;·&nbsp; LSTM Neural Network</div>
  <div class="hero-tags">
...
if "result" not in st.session_state:
    ...
    for col, num, title, body in [
        ...
        (c2, "02", "Run Model",  "Click RUN PREDICTION — all sidebar values are passed live to the LSTM."),
        (c3, "03", "Analyze",    "Explore Price, Indicators, and Diagnostics tabs."),
    ]:
        with col:
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.info("💡 Both `model.py` and `app.py` must be in the same folder. "
            "Every sidebar setting is passed directly to `run_pipeline(config)` — "
            "no need to edit model.py manually.")


# ─────────────────────────────────────────────
# RUN BUTTON — passes full UI config to model
# ─────────────────────────────────────────────
if run_btn:
    ui_config = {
        "ticker"      : ticker,
    ...
    with st.spinner(""):
        ...
        try:
            ...
            # Patch train to surface live epoch progress
            ...
            if raw["is_real_data"]:
                st.success(f"✅ Real data loaded — {len(raw['df'])} trading days from Yahoo Finance.")
            else:
                st.warning("⚠️ Using simulated data. Tick 'Use Yahoo Finance' for real predictions.")
...
if "result" in st.session_state:
    ...
    with tab2:
        ...
        st.markdown('<div class="section-label">// RAW DATA (last 60 rows)</div>',
                    unsafe_allow_html=True)
        disp = ["Close","Open","High","Low","Volume",
                "RSI","MACD","BB_Upper","BB_Lower","Momentum","Volatility"]
        st.dataframe(df[disp].tail(60).round(2),
    ...
    with tab3:
        ...
        with st.expander("Model Architecture"):
            st.code(f"""
Input  →  (batch, {sl_s}, 17 features)
──────────────────────────────────────────────────────
Bidirectional LSTM ({l1_s})     return_sequences=True
BatchNormalization
Dropout({dr_s})
LSTM ({l2_s})                    return_sequences=False
BatchNormalization
Dropout({dr_s})
Dense (64, relu)
Dense (32, relu)
